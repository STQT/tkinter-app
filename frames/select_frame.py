...
from utils.functions import del_nan, OpenListbox
...
class SelectFrame:
    ...
    def set_frame_middle(self):
        ...
        master = frame_midd_2
        ??????????????????????????????????
        self.widget_container = OpenListbox(master, *args, **kwargs)
        ...
        # list_box_3.bind("<<Enter>>", self.srch_in_listbox(sorted(self.actor_names)))
        # lb.bind("<<Leave>>", func_leave)
        #list_box_3.bind("<1>", self.srch_in_listbox)
